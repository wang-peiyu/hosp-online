...
public class HospApiController {
    ...
    {
        Page<Hospital> hospitalPages =hospitalService.selectHospPage(page, limit, hospitalQueryVo);
        return Result.ok(hospitalPages);
    }


    @ApiOperation(value="跟据医院名称模糊查询")
    ...
    @ApiOperation(value="跟据医院编号获取科室")
    @GetMapping("/department/{hoscode}")
    public Result index(@PathVariable String hoscode){
        List<DepartmentVo> deptTree = departmentService.findDeptTree(hoscode);
        return Result.ok(deptTree);
    }
    @ApiOperation(value="跟据医院编号获取医院预号详情")
    @GetMapping("/findHospDetail/{hoscode}")
    public Result getHospDetailByHosCode(@PathVariable String hoscode){
